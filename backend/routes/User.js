...
userRouter.post("/register", (req, res) => {
  ...
  User.findOne({ username }, (err, user) => {
    if (err)
      res
        .status(500)
        .json({ message: { msgBody: "Err occured", msgErr: true } });
    if (user)
      //if there is aleady a user in this name
      res.status(400).json({
        message: { msgBody: "Username is alrady taken", msgErr: true },
      });
    else {
...
userRouter.get(
  ...
  (req, res) => {
    if (req.user.role === "admin") {
      ...
      res
        .status(403)
        .json({ message: { msgBody: "youre a=not an admin", msgError: true } });
    }
  }
);
